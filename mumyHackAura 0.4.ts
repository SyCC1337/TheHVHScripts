...
class mumyHackAura {
    ...
    private prevTargetSet = new HashSet<JavaInteger>();
    private targetList = <Entity[] | null>[];
    ...
    private readonly serverPacketTimer = new MSTimer();
        ...
        if ((this.settings.attackTiming.get() & 8) !== 0) {
            ...
            if (this.settings.targetMode.get() === "Multi") {
                ...
                for (let entity of this.targetList!) {
                    RenderUtils.drawPlatform(entity, this.getDistanceToHitEntity(entity) < range && !(limit > 0 && ++count > limit) ? this.attackStateColor : this.waitStateColor);
                }
            } else {
                for (let entity of this.targetList!) {
                    RenderUtils.drawPlatform(entity, entity === this.target ? this.attackStateColor : this.waitStateColor);
                }
            }
        }
    }
    ...
    private updateTarget() {
        ...
        if (limitedNetworkPing > 0 && this.serverPacketTimer.hasTimePassed(limitedNetworkPing)) {
            return;
        }
        const player = mc.thePlayer!;
        ...
        switch (this.settings.priority.get()) {
            ...
            case "LivingTime":
                ...
                break;
        } if (prevTarget != null && this.settings.targetMode.get() === "Switch" && this.targetList.indexOf(prevTarget) !== -1 && this.getDistanceToHitEntity(prevTarget) < range) {
    ...
    private static AutoBlock = class AutoBlock {
        ...
        private static readonly blockPos = new BlockPos(0, 0, 0);
        public change = false;
        public status = false;
    ...
    public static ValuesContainer = class ValuesContainer {
        ...
        private writeSettings(object: { [key: string]: unknown }, next: boolean = false) {
            for (let key in object) {
                ...
                this.writeSettings(object, true);
            }
        }
...
class mumyScript {
    ...
    public static registeredModulesField = Script.class.getDeclaredField("registeredModules")!;
    public static registeredCommandsField = Script.class.getDeclaredField("registeredCommands")!;
    ...
    private static $ = mumyScript.$static$();
